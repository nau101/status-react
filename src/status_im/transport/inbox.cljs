...
(defn request-inbox-messages
  ...
  (log/debug "offline inbox: request-messages for topics " topics " from " from " to " to)
  (loop [from       from
         current-to to]
    (let [current-to (if (> (- to from) one-day)
          ...
          opts       (merge {:mailServerPeer (:address wnode)
                             ...
                             :to             current-to})]
      (doseq [topic topics]
        (log/info "offline inbox: request-messages request for topic " topic " from " from " to " current-to)
